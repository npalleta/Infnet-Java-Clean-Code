...
public class Main {
    ...
    private static final String VALOR_MENSALIDADE = "99.98";
    ...
    public static void main(String[] args) {
        ...
        out.println("\n--- Exercício 04 ---");
        BigDecimal todosPagtosSomados = pagamentos
            .stream()
        ...
        OptionalInt contadorProdutosTotaisVendidos = pagamentos
            .stream()
            .mapToInt(
                pagamento -> pagamento.getProdutos().size()
            )
            .reduce(Integer::sum);
        ...
        Produto prodCadeiraGamer = new Produto(
            "Cadeira Gamer",
            Paths.get("cadeiras/gamer"),
            new BigDecimal("1000.00")
        );
        Produto pacoteDeImagens = new Produto(
            "Pacote de Imagens - NFT",
            Paths.get("nfts/pacote_imagens"),
        ...
        pagamentos.forEach(pagamento -> {
            ...
            BigDecimal somaProdutosComprados = pagamento
                .getProdutos()
                .stream()
        ...
        Optional<Map.Entry<String, BigDecimal>> clienteQueGastouMais = mapClienteProdutosOrdenado
            .entrySet()
            .stream()
            .findFirst();
        if (clienteQueGastouMais.isPresent())
            out.printf(
                "Nome do Cliente: %s | Valor em Compra(s): %s%n",
                clienteQueGastouMais.get().getKey(),
                clienteQueGastouMais.get().getValue()
            );
        //

        Map<Cliente, List<List<Produto>>> clienteParaProdutos = pagamentos
            .stream()
            .collect(
        ...
        Map<Cliente, List<Produto>> flatMapClienteParaProdutos = clienteParaProdutos
            .entrySet()
            .stream()
            .collect(
        ...
        List<Map.Entry<Cliente, BigDecimal>> ordenaClientePorGasto = topClientes
            .entrySet()
            .stream()
            .sorted(
            ...
            .toList();
        //
        out.printf("Cliente com o maior gasto: %s%n", ordenaClientePorGasto.stream().findFirst().isPresent());
        ...
        out.println("\n--- Exercício 09 ---");
        Assinatura assinaturaAndre = new Assinatura(new BigDecimal(VALOR_MENSALIDADE), new Periodo(ontem, hoje),
            Plano.TRIMESTRAL, andre, true);
        Assinatura assinaturaCamila = new Assinatura(new BigDecimal(VALOR_MENSALIDADE),
            new Periodo(anoRetrasado, anoPassado), Plano.SEMESTRAL, camila, false);
        Assinatura assinaturaJonas = new Assinatura(new BigDecimal(VALOR_MENSALIDADE), new Periodo(anoRetrasado),
            Plano.ANUAL, jonas, false);
        Assinatura assinaturaMaria = new Assinatura(new BigDecimal(VALOR_MENSALIDADE), new Periodo(hoje, anoQueVem),
            Plano.SEMESTRAL, maria, false);
        ...
        out.println("\n--- Exercício 10 ---");
        long periodoAssinaturaAndre = ChronoUnit.MONTHS.between(assinaturaAndre.getPeriodo().getBegin(),
            assinaturaAndre.getPeriodo().getEnd().orElse(hoje));
        out.printf("O período de assinatura da André é de %s meses %n", periodoAssinaturaAndre);

        long periodoAssinaturaMaria = ChronoUnit.MONTHS.between(assinaturaMaria.getPeriodo().getBegin(),
            assinaturaMaria.getPeriodo().getEnd().orElseGet(() -> hoje));
        ...
        assinaturas
            ...
            .forEach(contagemPeriodo -> out.printf(
        ...
        //
        out.println();
        assinaturas.forEach(assinatura -> out.print(assinatura.getCliente().bloqueiaCompra(assinatura, pagamentos)));
    }
}